...
- **Backups include KvK alliance access keys in plaintext** — if KvK Prep is set up, an export file is as sensitive as the keys themselves; treat exported files as secrets, the same as you'd treat the sign-in links.
