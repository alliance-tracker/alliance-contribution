...
describe("the viewer tier is read-only", () => {
  it("returns role viewer for the viewer key", async () => {
    ...
    expect(await res.json()).toEqual({ role: "viewer", scheduler: true, kvk: { enabled: false } });
  });
...
describe("GET /api/auth/me", () => {
  it("returns role admin for the admin key", async () => {
    ...
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ role: "admin", scheduler: true, kvk: { enabled: false } });
  });
  ...
  it("returns role manager for the manager key", async () => {
    ...
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ role: "manager", scheduler: true, kvk: { enabled: false } });
  });

  it("returns role null when no key is presented", async () => {
    const res = await SELF.fetch("https://example.com/api/auth/me");
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ role: null, scheduler: true, kvk: { enabled: false } });
  });
