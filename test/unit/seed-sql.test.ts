...
describe("buildSeedSql", () => {
  ...
  it("seeds the ten default activity types with correct weights and instance caps", () => {
    ...
    expect(activityTypes?.match(/^\s*\(\d+, '/gm)).toHaveLength(10);
  });

  it("seeds the default scoring tiers (core bands per docs/specs/02_scoring.md §1)", () => {
    const tiers = stmts[1];
    ...
    expect(tiers?.match(/\(\d+, \d+, \d+\)/g)).toHaveLength(22);
  });
});
