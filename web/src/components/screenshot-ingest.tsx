...
export function ScreenshotIngest({
  ...
  disabled?: boolean;
  /** Disables the file picker (choose / drop zone / add more) without touching the mode switch. */
  ...
  const pickBlocked = screenshotsDisabled || pickerDisabled;

  // Abort any in-flight read if the component unmounts (e.g. the dialog is closed mid-batch).
  useEffect(() => {
  ...
  const resume = () => { setModeState("screenshots"); setExhausted(false); dispatch({ type: "resume" }); };
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        {usage && (
          <div className="flex min-w-0 flex-1 flex-col gap-1.5">
            <div className={cn("flex justify-between gap-2.5 text-[11.5px] leading-snug", meter === "low" ? "text-warn" : "text-muted")}>
              <span>{meterLine}</span>
              {/* The used-up sentence already states the reset time — no need to repeat it here. */}
              {meter !== "used_up" && (
      ...
      {/* Empty-state picker only makes sense in screenshot mode. The batch card below (and "Add
          more") stay visible in both modes once there are items — a 429 forcing paste mode must
          not strand the operator with no list and no Remove. */}
      {mode === "screenshots" && state.items.length === 0 && (
